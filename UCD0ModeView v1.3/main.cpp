	...
	switch(dwStep)
	{
		...
		case STEP_SWITCH_MODE_TIMEOUT:
			...
			if (!USBConn->CheckListBoxData(TListCheck_DevUSB, lbxUSB, false)
				...
				dwNotFindCount++; // 找不到裝置
				Delay(1000);
				if(dwNotFindCount == 10) {
					DEBUG("(!)NOT_FIND_DEV");
		...
		case STEP_CHECK_MODE:
			...
			if(bKeyBoardSwitch[dwModeIndex])
			{
				...
				WAIT_KEYBOARD_MSG("雙擊 Shift 切換模式");
			}
			else if(bOtherSwitch[dwModeIndex])
			{
				DEBUG_NETWORD("STEP_BUTTON_SWITCH");
				WAIT_KEYBOARD_MSG(OtherSwitchMessage[dwModeIndex]);
			}
		...
		case STEP_WAITING_TEST_RESULT:
			...
			if(strRecBuf.Pos("[AS]MODE_PASS"))
			{
				...
				{
					dwNotFindCount = 0;
					if(plErrorMsgWindows->Visible == true)
						plErrorMsgWindows->Visible = false;
					dwTimeOut = 0;
					//目前MODE確認
		...
		case STEP_CHECK_SCREEN:
			if (dwScreenResult == RESULT_SCREEN_PASS) {//PASS
				...
				{
					DEBUG_NETWORD("STEP_REPLUG_SWITCH");
					dwTimeOut = GetTickCount() + dwKeyBoardSwitchTimeOut[dwModeIndex];
					dwStep = STEP_REPLUG_SWITCH;
					WAIT_KEYBOARD_MSG(OtherSwitchMessage[dwModeIndex]);
				}
				else
				{
...
DWORD TfrmMain::CheckDevMode(int ModeIndex) {
	...
	if (ModeIndex == 0) {
		...
		if (plScreenInfo->Caption != TListScreen_Dev[ModeIndex - 1]->Text.Trim()
			...
			ChangeMsgBarUI(false, 4, clGreen);
		if (NETWORKCARD_MAC_CHECK[ModeIndex-1] != "" && NETWORKCARD_MAC_CHECK[ModeIndex-1] != "-")
		{
			plMAC->Caption = USBConn->getLanMAC(NETWORKCARD_MAC_CHECK[ModeIndex-1]);
			if (plMAC->Caption == "" || plMAC->Caption == "-")
			{	
				bPass = false;	
				plMAC->Font->Color = clRed;	 
			}
			else
				plMAC->Font->Color = clBlack;	 	
		}
		else
			plMAC->Caption = "";
		if (bPass)
...
void TfrmMain::LoadINI() {
	...
	for (int i = 0; i < 5; i++) {
		...
		bOtherSwitch[i]  = ini->ReadBool(strTemp,"OTHER_SWITCH",false);
		OtherSwitchMessage[i] = ini->ReadString(strTemp, "OTHER_SWITCH_MESSAGE","按下 按鈕 切換模式");
		...
		if (i > 0) {
			...
			NETWORKCARD_MAC_CHECK[i-1] = ini->ReadString(strTemp, "NETWORKCARD_MAC_CHECK",
				"-");
		}
	}
	delete ini;
...
bool TfrmMain::CheckPDVersion()
{
	...
	astrNowPDVersion = StringReplace(astrNowPDVersion, "\r", "", TReplaceFlags() << rfReplaceAll);
	astrNowPDVersion = StringReplace(astrNowPDVersion, "\n", "", TReplaceFlags() << rfReplaceAll);
	astrNowPDVersion = astrNowPDVersion.Trim();
